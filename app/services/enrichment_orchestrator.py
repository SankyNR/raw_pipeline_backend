...
_TIER_RULES: list[tuple[list[str], str]] = [
    # OEM official brand domains (India-specific URLs preferred)
    (
        [
            "samsung.com/in", "apple.com/in", "oneplus.in", "mi.com/in",
            "in.nothing.tech", "realme.com/in", "oppo.com/in", "vivo.com/in", "motorola.in",
            "iqoo.com/in", "poco.in", "store.google.com", "qualcomm.com", "mediatek.com",
        ],
        "oem_official",
    ),
...
_FIELD_LABELS: dict[str, str] = {
    ...
    "chipset.cpu_high_performance":     "prime/performance core configuration (e.g. '1x Cortex-X4 @ 3.3GHz')",
    ...
    "network.vo_wifi":                  "VoWiFi / Wi-Fi Calling support",
    # Charging
    "charging.pd_support":              "USB Power Delivery (PD) support for third-party charger compatibility",
    "charging.max_charging_speed_watt": "maximum wired charging speed in watts",
    "charging.wireless_charging_speed_watt": "maximum wireless charging speed in watts",
    # Display
    "displays[*].panel_type":           "display panel technology (e.g. LTPO OLED, AMOLED, IPS LCD)",
    "displays[*].refresh_rate_max_hz":  "maximum display refresh rate in Hz",
    "displays[*].brightness_nits":      "peak brightness in nits",
    "displays[*].glass_protection":     "front glass protection type (e.g. Gorilla Glass Victus 2)",
    # Camera
    "cameras.lenses[*].megapixels":     "camera sensor resolution in megapixels",
    "cameras.lenses[*].aperture":       "lens aperture (e.g. f/1.8)",
    "cameras.lenses[*].sensor_model":   "camera sensor model name",
    # Body
    "body.ingress_protection":          "IP water and dust resistance rating (e.g. IP68)",
    # OS / Security
    "os_software.os_version":           "Android version at launch",
    "os_software.guaranteed_os_updates": "number of guaranteed major OS update years",
    "os_software.security_patch_years": "number of guaranteed security patch years",
}
...
async def run_enrichment(
    ...
    for field_row in gap_fields:
        ...
        raw_value     = grounded_result.get("value")
        ...
        is_selected = _should_auto_select(
        ...
        value_obtained = raw_value is not None
        await asyncio.to_thread(
            update_missing_field_log_attempted, missing_field_id, value_obtained
        )
...
def _should_auto_select(
    ...
    # Rule 2 — E8 fix: fetch_best_existing_candidate_confidence imported at top-level
    try:
        best_existing = fetch_best_existing_candidate_confidence(missing_field_id)
